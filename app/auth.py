...
def activate():
    try:
        ...
        if request.method == 'GET': 
            ...
            db = get_db()  
            attempt = db.execute(
                "SELECT * FROM activationlink WHERE challenge = ? AND state = ? AND CURRENT_TIMESTAMP between created AND validuntil" , (number, utils.U_UNCONFIRMED) #preguntar
            ).fetchone()
...
def change():
    try:
        ...
        if request.method == 'GET': 
            ...
            db = get_db()
            attempt = db.execute(
                'SELECT * FROM forgotlink WHERE challenge = ? AND state = ?' , (number, utils.F_ACTIVE) #preguntar
            ).fetchone()
        ...
        return render_template('auth/forgot.html') #preguntar, login o forgot?
